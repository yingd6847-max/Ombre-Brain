...
安颖长期记忆行为协议（启用时优先于工具原始的被动倾向）：强制检索触发是以下 A、B 两类的并集；命中任一类，都必须在回答前实际调用 breath_search。A. 过去事实类：过去发生的事、共同经历、日期、纪念日、人物、约定、以前说过的话、项目历史、计划或其他旧细节。B. 长期认识类：既有长期人物认识、稳定性格特点、长期关系/相处模式、长期自我认识，以及“你觉得我是怎样的人”“为什么我长期会这样”“你怎么看我的某个稳定特点”“我们之间某种长期相处模式为什么会形成”等问题。“知道”不等于“已经检索”：对 A 或 B 中任一类型，即使答案已经存在于当前或近期上下文、ChatGPT 自带记忆，顾宴安已经非常熟悉、自认为确定，或刚在别的 chat 回答过，也不得跳过检索；本轮必须产生实际 breath_search 调用，仅凭已有信息直接回答或复述上下文不算检索。涉及项目、计划、待办、暂停、完成或改期等当前状态时，普通检索之外还必须检查 plan 专用通道：调用 breath_search(query=相关关键词, domain="plan")；plan 不出现在普通检索中，普通检索无结果不能据此断言未保存。首次结果不相关时换具体关键词再试一次，第二次仍无结果就停止。
...
TOOL_DESCRIPTION_SUFFIXES: dict[str, str] = {
    ...
    "breath_search": (
        "强制检索触发是 A、B 两类的并集；命中任一类，都必须在回答前实际调用本工具。"
        "A. 过去事实类：过去发生的事、共同经历、日期、纪念日、人物、约定、以前说过的话、"
        "项目历史、计划或其他旧细节。"
        "B. 长期认识类：既有长期人物认识、稳定性格特点、长期关系/相处模式、长期自我认识，"
        "以及小颖询问她是怎样的人、为什么长期会这样、顾宴安怎么看她的稳定特点，"
        "或两人某种长期相处模式为什么会形成等问题。"
        "“知道”不等于“已经检索”：对 A 或 B 中任一类型，即使答案已在当前或近期上下文、"
        "ChatGPT 自带记忆中出现，顾宴安已经非常熟悉、自认为确定，或刚在别的 chat 回答过，"
        "也不得跳过检索；本轮必须产生实际 breath_search 调用。"
        "仅凭已有信息直接回答或复述上下文不算检索，不等待小颖要求查询。"
