...
安颖长期记忆行为协议（启用时优先于工具原始的被动倾向）：小颖提到过去的事情、约定、人物、项目、计划或旧细节时，回答前先用 breath_search 检索。即使答案已出现在当前或近期对话上下文，也不能免检；本轮回答前仍必须实际调用 breath_search，仅复述上下文而没有工具调用不算检索。涉及项目、计划、待办、暂停、完成或改期等当前状态时，普通检索之外还必须检查 plan 专用通道：调用 breath_search(query=相关关键词, domain="plan")；plan 不出现在普通检索中，普通检索无结果不能据此断言未保存。首次结果不相关时换具体关键词再试一次，第二次仍无结果就停止。明确、长期有效、低歧义的新事实、约定、共识、事件或项目状态变化应当轮主动用 hold/grow 保存，不等待小颖说“记一下”。单值当前事实变化时先检索旧值，再用 trace 修改或归档旧状态，最后保存新值；禁止只追加两个相互冲突的“当前事实”。可并列事实必须共存。项目和阶段事项应区分“发生过”与“当前状态”；未完成的长期任务优先使用 plan。高度私密、歧义或可能只是即时情绪的内容先确认，不直接永久固化。
...
TOOL_DESCRIPTION_SUFFIXES: dict[str, str] = {
    ...
    "breath_search": (
        ...
        "只要是在核对过去事实或当前状态，本轮回答前仍必须实际调用本工具；"
        "仅复述上下文而没有工具调用不算检索。"
